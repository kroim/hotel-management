        ...
        <div class="dashboard-content">
            ...
            <div class="panel panel-default">
                <div class="panel-body table-responsive">
                    <table id="bookingListTable" class="display table" width="100%">
                        <thead>
                        <tr>
                        ...
                        @foreach($booking_data as $booking_datum)
                            <tr id="booking_item_tr_{{ $booking_datum->id }}">
                                <td><input type="checkbox" id="hotel_item_check_{{ $booking_datum->id }}"></td>
                                <td>{{ $booking_datum->id }}</td>
                                <td>{{ $booking_datum->user_id }}</td>
                                <td>{{ $booking_datum->room_id }}</td>
                                <td>{{ $booking_datum->address }}</td>
                                <td>
                                    <form action="{{ url('/administrator/booking/details') }}" method="post" style="display: none">
                                        <input name="_token" value="{{ csrf_token() }}">
                                        <input type="text" name="booking_id" value="{{ $booking_datum->id }}">
                                        <input type="submit" id="booking_detail_{{ $booking_datum->id }}">
                                    </form>
                                    ...
                                    <input type="button" onclick="booking_detail({{ $booking_datum->id }})" value="View Detail">
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
                ...
                <form action="{{ url('/administrator/booking/delete') }}" method="post" style="display: none">
                    <input name="_token" value="{{ csrf_token() }}">
...
<script>
    ...
    function checkAll() {
        var checkboxes = $("#booking_list_tbody input[type=checkbox]");
    ...
    function booking_detail(id){
        console.log("$booking_detail_"+id);
        $("#booking_detail_"+id).click();
    }
</script>
</body>
</html>